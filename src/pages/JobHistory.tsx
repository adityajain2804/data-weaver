...
import { Eye, ChevronDown, Download, Package, TrendingDown, TrendingUp, X } from "lucide-react";
import { StatusBadge } from "@/components/StatusBadge";
import { Button } from "@/components/ui/button";
import { ProductDetailDialog } from "@/components/ProductDetailDialog";
import { listJobs, stopJob, subscribeJobs } from "@/lib/job-store";
...
export default function JobHistory() {
  ...
  const statusKey = (s?: string) => (s || "pending").toLowerCase();
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <div className="grid grid-cols-[120px_1fr_auto_auto_auto_1fr_auto] gap-4 border-b border-border bg-muted/50 px-5 py-2.5 text-[11px] font-medium uppercase tracking-wider text-muted-foreground">
          <span>Job ID</span>
          <span>URL</span>
          <span>Fields</span>
          <span>Config</span>
        ...
        <div className="divide-y divide-border">
          {jobs.length === 0 && (
            <div className="px-5 py-8 text-center text-[13px] text-muted-foreground">No jobs found.</div>
          )}

          {jobs.map((job) => {
            const isCompleted = statusKey(job.status) === "completed" || statusKey(job.status) === "success";
            const isFailed = statusKey(job.status) === "failed";
            const isRunning = statusKey(job.status) === "running";

            return (
              <div key={job.id}>
                <div className="grid grid-cols-[120px_1fr_auto_auto_auto_1fr_auto] gap-4 items-center px-5 py-3 transition-colors hover:bg-muted/30">
                  <button
                    onClick={() => navigate(`/job/${job.id}`)}
                    className="font-mono text-[12px] text-muted-foreground hover:text-primary text-left truncate"
                  ...
                  <span className="rounded-md bg-muted px-2 py-0.5 text-[11px] font-medium text-muted-foreground whitespace-nowrap">
                  ...
                  <div className="min-w-0">
                    {(isFailed || job.last_error) && (
                      <p className="max-w-[320px] truncate text-[10px] leading-4 text-destructive" title={job.last_error || job.error || ""}>
                  ...
                  <div className="flex items-center gap-2">
                    {isRunning && (
                      <Button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          stopJob(job.id);
                          toast.success("Job stop request sent");
                        }}
                        className="h-7 text-[11px] px-2 border-destructive/30 text-destructive hover:bg-destructive/10 shadow-none"
                      >
                    ...
                    {isCompleted && (
                      <Button
                        variant="outline"
                        size="sm"
                        title="Download Silver layer as Parquet"
                        className="h-7 text-[11px] px-2 border-primary/30 bg-primary/5 text-primary hover:bg-primary/10 shadow-none"
                      >
                ...
                <AnimatePresence>
                  {expandedJob === job.id && isCompleted && (
                    <motion.div
                      ...
                      <div className="border-t border-border bg-muted/20 px-5 py-4 space-y-3">
                        <div className="flex items-center justify-between">
                          <span className="text-[12px] font-medium text-foreground">
                            Silver Table Results ({mockProductData.length} items)
                          </span>
                          <div className="flex items-center gap-2">
                            ...
                            <Button variant="outline" size="sm" className="text-[10px] h-6 px-2 border-primary/30 bg-primary/5 text-primary hover:bg-primary/10">
                              <Download className="mr-1 h-3 w-3" /> Export Parquet
                            </Button>
                          </div>
                        </div>
                        ...
                        {resultView === "cards" ? (
                          <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                            {mockProductData.slice(0, 6).map((product, i) => {
                                ...
                                >
                                  <div className="flex items-start gap-2">
                                    ...
                                    <div className="min-w-0 flex-1">
                                      <p className="text-[12px] font-medium text-foreground leading-tight group-hover:text-primary transition-colors truncate">{product.title}</p>
                                  ...
                                  <div className="flex items-center justify-between">
                                    ...
                                    <StatusBadge status={product.availability === "In Stock" ? "completed" : "pending"} />
                                  </div>
                                </button>
                              );
                            })}
                  ...
                  {expandedJob === job.id && isFailed && (
                    ...
                    >
                      <div className="border-t border-border bg-destructive/5 px-5 py-4">
                        <p className="text-[12px] font-medium text-destructive">{job.last_error || job.error || "Unknown error occurred"}</p>
                      </div>
                    </motion.div>
                  )}
                </AnimatePresence>
